...
const EXAMPLE = '你好，Tool Forge 工具箱'
...
export default function UnicodeCodec() {
  ...
  const { output, error } = useMemo(() => {
    try {
      const value = mode === 'encode' ? textToUnicode(input) : unicodeToText(input)
  ...
  return (
    ...
    >
      <div className="grid h-full grid-cols-1 gap-4 lg:grid-cols-2">
        <TextPanel
          label={mode === 'encode' ? '原文' : '\\uXXXX 字符串'}
          value={input}
          onChange={setInput}
        />
        <TextPanel
          label={mode === 'encode' ? '\\uXXXX 字符串' : '原文'}
          value={output}
          error={error}
          readOnly
        />
