...
type Summary = codexinsight.SkillSummary
type SkillFile = codexinsight.SkillFile
type FileContent = codexinsight.SkillFileContent
...
function SkillsList({
  ...
  if (loading && !items) {
    return <Loading text="正在扫描 ~/.codex/skills ..." />
  }
    ...
    <div className="flex h-64 flex-col items-center justify-center gap-3 rounded-lg border border-dashed border-border bg-card text-center">
      ...
      <div className="space-y-1">
        ...
        <p className="max-w-md text-xs text-muted-foreground">
          skill 是一段可复用的指令或工作流,Codex 会根据它调整行为。
        </p>
      </div>
...
function SkillDetail({
  ...
  const doCreateFile = async (rel: string) => {
    ...
    try {
      setBusy(true)
      await WriteCodexSkillFile(skill, rel, '')
      load()
    } catch (e) {
      setError(e instanceof Error ? e.message : String(e))
    } finally {
  ...
  const doDeleteFile = async () => {
    ...
    try {
      setBusy(true)
      await DeleteCodexSkillFile(skill, path)
      load()
    } catch (e) {
      setError(e instanceof Error ? e.message : String(e))
    } finally {
...
function FileEditor({
  ...
  const save = async () => {
    ...
    try {
      setSaving(true)
      await WriteCodexSkillFile(skill, path, content)
      setOriginal(content)
      setToast('已保存')
